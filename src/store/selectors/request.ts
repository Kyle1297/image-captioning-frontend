...
export const requestsInProgress = (state: AppState) => (
  requestState(state).requests.filter(request => request.isLoading).length > 0
);

// get requests in progress either by single requestName or by requestNames array 
export const namedRequestsInProgress = (state: AppState, requestName: number | number[]) => {
  const singleNamedRequestInProgress = (singleRequestName: number) =>
    requestState(state).requests.find(request => 
			request.name === singleRequestName && request.isLoading) !== undefined
...
export const namedRequestError = (state: AppState, requestName: number) => (
  requestState(state).requests.find(request =>
    request.name === requestName && request.error !== null)?.error
);